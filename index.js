...
function processSum(numberList, callback) {
   const totalSum = numberList.reduce(function(num1, num2){
    return num1 + num2 
   }, 0);
...
function getFullNames(runners) {   
  ...
  const allRunners = [];
    runners.forEach(function(currentValue) {
      return allRunners.push(`${currentValue.last_name}, ${currentValue.first_name}`)
    });
...
function firstNamesAllCaps(runners) {
  ...
  runners.map(function (currentValue){
    return nameAllCaps.push(`${currentValue.first_name.toUpperCase()}`);
  });
  return nameAllCaps;
}
